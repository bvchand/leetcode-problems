class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        while(left <= right) {
            ...
            int cut2 = (totalLen)/2 - cut1;           // no. of elements in nums2
            ...
            if(left1 <= right2 && left2 <= right1)
                return (totalLen % 2 == 0) ? ((double) Math.max(left1, left2) +  (double) Math.min(right1, right2)) / 2 : (double) Math.min(right1, right2);
